...
public class CSVFilterData {
	...
	public void filterCSVDataWithBinarySuccess(String fileLocation, ArrayList<String> filter,
		...
		File file = new File(String.format("%s/filtered_data-%s.csv", filePath.getParent(),
				filePath.getName().replaceFirst("[.][^.]+$", "")));
		try {
...
//		System.out.print("The feature column id is : " + columnFeature.get(featureNameForMedianSplit.toLowerCase()));
		...
		File file = new File(String.format("%s/binary_success_data-%s.csv", filePath.getParent(),
				filePath.getName().replaceFirst("[.][^.]+$", "")));
		try {
			// create FileWriter object with file as parameter
			FileWriter outputfile = new FileWriter(file);
	...
	public void convertCSVToARFFNominal(String fileLocation) throws Exception {
